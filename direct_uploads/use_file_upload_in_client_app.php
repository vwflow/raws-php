<?php
# Sample to demonstrate how to upload a file directly to the CDN using HMAC authentication.
# This is done using the RASS 'file_upload_m' resource.
# For more info, see https://wiki.rambla.be/RASS_file_upload_m_resource
#
...
try {
  # no pwd needed, since rass->doFileUpload() sends a HMAC based on the $secret
  $rass = new RassService(USER, null, RASS_SERVER); 
  
  # RASS PUT file_upload
  echo "\nUploading file to CDN, please wait..";
  $res = $rass->doFileUpload($filename, $local_path, $secret, $seconds_valid, $publish_dir);
