<?php
# Sample to demonstrate how to send a RATS batch job, publish the encoded files on the CDN, and retrieve detailed info.
#
...
try {
  ...
  $job = $rats->createBatchJob(RATS_FORMATGROUP_MP4_JPG_480P,         # formatgroup, which contains an mp4 + jpg profile
                              $src->entry->content->params->filename, # name of the RATS src we've just uploaded
                              "test/rats_batch_job/mymovie",          # location on the CDN for publishing (mymovie = filename, RATS adds the right extension)
                              "40",                                   # create snapshots at every 40% of the video (=> at 0%, 40% and 80%)
